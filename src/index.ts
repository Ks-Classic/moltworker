...
function validateRequiredEnv(env: MoltbotEnv): string[] {
  ...
  if (!hasCloudflareGateway && !hasLegacyGateway && !hasAnthropicKey && !hasOpenAIKey && !hasGeminiKey) {
    missing.push(
      'ANTHROPIC_API_KEY, OPENAI_API_KEY, GEMINI_API_KEY, or CLOUDFLARE_AI_GATEWAY_API_KEY + CF_AI_GATEWAY_ACCOUNT_ID + CF_AI_GATEWAY_GATEWAY_ID',
    );
  }
...
app.all('*', async (c) => {
  ...
  try {
    ...
    const hasAnyProvider = !!(c.env.ANTHROPIC_API_KEY || c.env.OPENAI_API_KEY || c.env.GEMINI_API_KEY ||
      (c.env.CLOUDFLARE_AI_GATEWAY_API_KEY && c.env.CF_AI_GATEWAY_ACCOUNT_ID && c.env.CF_AI_GATEWAY_GATEWAY_ID));
    if (!hasAnyProvider) {
      hint = 'No AI provider configured. Run: wrangler secret put GEMINI_API_KEY (or ANTHROPIC_API_KEY / OPENAI_API_KEY)';
