...
app.all('*', async (c) => {
  ...
  try {
    ...
    if (!c.env.ANTHROPIC_API_KEY && !c.env.OPENAI_API_KEY && !c.env.GEMINI_API_KEY) {
      hint = 'No AI provider key is set. Run: wrangler secret put GEMINI_API_KEY (or ANTHROPIC_API_KEY / OPENAI_API_KEY)';
